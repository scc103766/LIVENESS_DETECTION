...
def collect_samples_from_label_dirs(root_dir: Path) -> list[tuple[str, str, int]]:
    ...
    for child in sorted(root_dir.iterdir()):
        ...
        for video_path in sorted(child.rglob("*")):
            if video_path.is_file() and video_path.suffix.lower() in VIDEO_EXTENSIONS:
                ...
                if txt_path.exists():
                    samples.append((str(video_path), str(txt_path), label))
    return samples
...
def stratified_split(
    ...
    grouped: dict[int, list[tuple[str, str, int]]] = {0: [], 1: []}
...
def parse_color_txt(txt_path: str) -> dict[int, int]:
    ...
    def _read_all_frames_with_color(self, video_path: str, txt_path: str) -> tuple[list[np.ndarray], list[np.ndarray]]:
        ...
        while True:
            ...
            if frame is not None and frame.size > 0:
                color_value = color_map.get(current_frame_idx, prev_color_value if prev_color_value is not None else 0)
...
def predict_video(args: argparse.Namespace) -> None:
    ...
    dataset = FlashLivenessDataset(
        samples=[(args.video_path, txt_path, 0)],
        transform=False,
        preprocessor=preprocessor,
        target_size=tuple(config["target_size"]),
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Flash liveness training and inference V2 (sequential frame reading)")
