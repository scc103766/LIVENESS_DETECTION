...
class MoE(nn.Module):
    ...
    def to(self, device):
        ...
        return super(MoE, self).to(device)
    ...
    def forward(self, x, task_id=2):
        ...
        # 做深度图---------------------------------------------------------------------
        single_depth_map_print, depth_soft_max_print = self.infer_depth_map(self.u_net_print, x) if (
                    task_id == 0 or task_id == 2) else (None, None)
        single_depth_map_screen, depth_soft_max_screen = self.infer_depth_map(self.u_net_screen, x) if (
                    task_id == 1 or task_id == 2) else (None, None)
        ...
        if self.infer_type == "depth":
            if task_id == 0:
                ...
                single_depth_map = single_depth_map_screen + single_depth_map_screen
            return single_depth_map.permute(0, 2, 3, 1)

        # 做分类-----------------------------------------------------------------------
        single_p_print = torch.reshape(self.f_net_print(single_depth_map_print), [-1, 2]) if (
        ...
        single_p_screen = torch.reshape(self.f_net_screen(single_depth_map_screen), [-1, 2]) if (
        ...
        if self.infer_type == "score":
            ...
            return sc_p, torch.log(depth_soft_max_print + 1e-5), single_p_print, single_pred_print, None
        elif task_id == 1:
            return sc_p, torch.log(depth_soft_max_screen + 1e-5), single_p_screen, single_pred_screen, None
        else:
            ...
            single_pred = torch.cat(
                [torch.unsqueeze(single_pred_print, dim=1), torch.unsqueeze(single_pred_screen, dim=1)], dim=1)
            single_pred, _ = torch.min(single_pred, dim=1)
            return sc_p, depth_soft_max, single_p, single_pred, None

    def infer_depth_map(self, u_net, x):
        depth_x = u_net(x)
        ...
        # 融合多帧深度图成单帧
        single_depth_map = torch.mean(
            torch.reshape(depth_map, [-1, self.frame_num, 64, 64]), dim=1,
            keepdim=True)
        return single_depth_map, depth_soft_max
