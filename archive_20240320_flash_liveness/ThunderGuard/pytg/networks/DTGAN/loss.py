...
def get_loss_module(device):
    ...
    contra_loss = contra_loss.to(device)
    return loss_sc, single_loss_cls, loss_depth, contra_loss
...
def build_loss(batch_data, loss_list, model, device, data_type=None):
    ...
    sc_p, depth_log_softmax, single_p, single_pred, depth_map_attention, corr = model(normal_cue)
    ...
    loss_sc, single_loss_cls, loss_depth, contra_loss = loss_list
    ...
    loss_dict = {
        "loss": loss_0 + loss_1 * 0.16 + loss_2 * 6.0,
        "color_loss": loss_0,
        "score_loss": loss_1,
        "map_loss": loss_2,
    }

    IS_USE_CONTRA_LOSS = False

    if IS_USE_CONTRA_LOSS:
        single_target = 1.0 - single_label
        loss_3 = contra_loss(corr, single_target)
        loss_dict["loss"] = loss_dict["loss"] + loss_3
        loss_dict["contra_loss"] = loss_3
