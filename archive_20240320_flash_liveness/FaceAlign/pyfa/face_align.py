...
parser.add_argument('--normalize_percent', type=float, default=0.95, help="亮度归一化参数，仅normalize_percent的像素参与归一化")
# parser.add_argument('--img_type', type=str, default='png', help="输出图谱格式")
...
def main():
    ...
    for file in tqdm(file_list):
        ...
        for index, (img, pos) in enumerate(zip(img_list, pos_list)):
            ...
            # 测试是否准确
            # plt.imshow(draw_kps(img, np.concatenate(
            #     [np.array(key_point_x).reshape([-1, 1]), np.array(key_point_y).reshape([-1, 1])], axis=1)))
            # plt.axis('off')
            # plt.show()

            assert left_eye_id == 68 and right_eye_id == 69
            slow_align_img, trans_pars = face_align.slow_align(img, key_point_x, key_point_y, left_eye_id, right_eye_id,
                                                               -1.0 if index == 0 else 1.0)

            # 计算深度图
            cropped_img = cv2.cvtColor(cv2.resize(slow_align_img, (256, 256)), cv2.COLOR_BGR2RGB) / 255.
            cropped_pos = pos_predictor.predict(cropped_img)
            all_vertices = np.reshape(cropped_pos, [256 * 256, -1])
            vertices = all_vertices[face_ind, :]
            depth = cv2.resize(face_align.cal_depth_face(vertices), (args.align_size, args.align_size))
            slow_align_img = np.concatenate([slow_align_img, depth[:, :, 0:1]], axis=2)

            align_img_list.append(slow_align_img)
            align_trans_list.append(trans_pars)
        align_img = np.concatenate(align_img_list, axis=0)
        cv2.imwrite(os.path.join(args.to_path, file.replace(".txt", ".png")), align_img[:, :, :3])
        cv2.imwrite(os.path.join(args.to_path, file.replace(".txt", "_d.jpg")), align_img[:, :, 3:])

        def write_key_point():
            key_point_file = os.path.join(args.to_path, file.replace(".txt", "_key_point.txt"))
            with open(key_point_file, 'w') as f:
                for key_point_x, key_point_y in zip(key_point_x_list, key_point_y_list):
                    line = ",".join(list(map(lambda a: str(a), key_point_x))) + ";" + ",".join(
                        list(map(lambda a: str(a), key_point_y)))
                    f.write(line + "\n")
        #write_key_point()

        # if args.img_type == "jpg":
        #     cv2.imwrite(os.path.join(args.to_path, file.replace(".txt", ".jpg")), align_img[:, :, :3])
        #     cv2.imwrite(os.path.join(args.to_path, file.replace(".txt", "_d.jpg")), align_img[:, :, 3:])
        # else:
        #     cv2.imwrite(os.path.join(args.to_path, file.replace(".txt", ".png")), align_img)
        #     key_point_file = os.path.join(args.to_path, file.replace(".txt", "_key_point.txt"))
        #     with open(key_point_file, 'w') as f:
        #         for key_point_x, key_point_y in zip(key_point_x_list, key_point_y_list):
        #             line = ",".join(list(map(lambda a: str(a), key_point_x))) + ";" + ",".join(
        ...
        if len(pos_list[0]) > 4:
            with open(os.path.join(args.to_path, file), "w") as f:
                for pos in pos_list:
                    new_pos_list = [pos[0]] + pos[5:]
                    f.write(",".join(list(map(lambda a: str(a), new_pos_list))) + "\n")
        # align_trans = np.concatenate(align_img_list, axis=0)
        # np.save(os.path.join(args.to_path, file.replace(".txt", "_fast_trans.npy")), align_trans)
