...
def detect(opt):
    ...
    if isinstance(imgsz, (list,tuple)):
        ...
        print(f"__height and width of image {imgsz}")
        imgsz[0] = check_img_size(imgsz[0], s=stride)
        imgsz[1] = check_img_size(imgsz[1], s=stride)
    else:
        imgsz = check_img_size(imgsz, s=stride)  # check img_size
        # print(f"{type(imgsz)}__height and width of image {imgsz}") # int  640
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img-size', nargs= '+', type=int, default=640, help='inference size (pixels)') # 640
