...
from yolov7_face.utils.autoanchor import check_anchor_order
from yolov7_face.utils.general import make_divisible, check_file, set_logging
from yolov7_face.utils.torch_utils import time_synchronized, fuse_conv_and_bn, model_info, scale_img, initialize_weights, \
...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None,logger_=None):  # model, input channels, number of classes
        ...
        if nc and nc != self.yaml['nc']:
            self.logger.info(f"Overriding model.yaml nc={self.yaml['nc']} with nc={nc}")
    ...
    def forward_once(self, x, profile=False):
        ...
        for m in self.model:
            ...
            if profile:
                ...
                if m == self.model[0]:
                    self.logger.info(f"{'time (ms)':>10s} {'GFLOPS':>10s} {'params':>10s}  {'module'}")
                self.logger.info(f'{dt[-1]:10.2f} {o:10.2f} {m.np:10.0f}  {m.type}')

            x = m(x)  # run
            y.append(x if m.i in self.save else None)  # save output

        if profile:
            self.logger.info('%.1fms total' % sum(dt))
        return x
    ...
    def nms(self, mode=True):  # add or remove NMS module
        ...
        if mode and not present:
            ...
            self.logger.info('Removing NMS... ')
            self.model = self.model[:-1]  # remove
        return self

    def autoshape(self):  # add autoShape module
        self.logger.info('Adding autoShape... ')
