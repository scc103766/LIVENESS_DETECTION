...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='/supercloud/llm-code/scc/scc/insightface/recognition/arcface_torch/xxt_run_dataset/yolov7_face/yolov7-w6-face.pt', help='weights path')
    ...
    opt.img_size *= 2 if len(opt.img_size) == 1 else 1  # expand这一行的主要作用是处理用户输入的灵活性，确保 opt.img_size 最终一定是一个包含两个元素 [height, width] 的列表。
    print(opt)
    set_logging()
    t = time.time()
    ...
    opt.img_size = [check_img_size(x, gs) for x in opt.img_size]  # verify img_size are gs-multiples确保图像的高度和宽度都满足模型的网络结构要求，即它们都必须是模型最大下采样步长（max stride）的整数倍
